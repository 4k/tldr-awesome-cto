...
-
73Summary: The upper bound of the complexity of an algorithm. See also the similar question Big O, how do you calculate/approximate it? for a good explaination.Kosi2801– Kosi28012009-01-28 11:18:44 +00:00Commented Jan 28, 2009 at 11:18
-
7The other answers are quite good, just one detail to understand it: O(log n) or similar means, that it depends on the "length" or "size" of the input, not on the value itself. This could be hard to understand, but is very important. For example, this happens when your algorithm is splitting things in two in each iteration.Harald Schilly– Harald Schilly2009-01-28 11:23:43 +00:00Commented Jan 28, 2009 at 11:23
-
20There is a lecture dedicated to complexity of the algorithms in the Lecture 8 of the MIT "Introduction to Computer Science and Programming" course youtube.com/watch?v=ewd7Lf2dr5Q It is not completely plain English, but gives nice explanation with examples that are easily understandable.ivanjovanovic– ivanjovanovic2010-07-17 20:57:40 +00:00Commented Jul 17, 2010 at 20:57
-
20Big O is an estimate of the worst case performance of a function assuming the algorithm will perform the maximum number of iterations.Paul Sweatte– Paul Sweatte2012-08-28 00:16:57 +00:00Commented Aug 28, 2012 at 0:16
-
40
Big-O notation explained by a self-taught programmer
Soner Gönül– Soner Gönül2013-09-07 18:02:25 +00:00Commented Sep 7, 2013 at 18:02
42 Answers
...
- the number of possible handshakes among
N
...
2 Comments
Hashtables are generally amortised (~= "on average") O(1) — once they're set up
? How does that happen? Even after creating hash of every key in the dictionary, dictionary data structure will have to do some binary look-up when I ask it for a key and its respective value pair. Isn't it?Big O describes an upper limit on the growth behaviour of a function, for example the runtime of a program, when inputs become large.
Examples:
...
space
and time
. For example, you could create an algorithm that requires a lot more space, which helps reduce your time complexity, or you could create a an algorithm that does not require any additional space, such as in situ
algorithms. People in practice would then pick the algorithm that best suits their need, need it be for performance or to minimize space, etc.It is very difficult to measure the speed of software programs, and when we try, the answers can be very complex and filled with exceptions and special cases. This is a big problem, because all those exceptions and special cases are distracting and unhelpful when we want to compare two different programs with one another to find out which is "fastest".
...
1 Comment
O(n log n)
which would be considered good.What is a plain English explanation of Big O? With as little formal definition as possible and simple mathematics.
...
1
step, it's excellent, Ordered No.1 - O(logN), means your computer complete a task with
logN
steps, its good, Ordered No.2 - O(N), finish a task with
N
steps, its fair, Order No.3 - O(NlogN), ends a task with
O(NlogN)
steps, it's not good, Order No.4 - O(N^2), get a task done with
N^2
steps, it's bad, Order No.5 - O(2^N), get a task done with
2^N
steps, it's horrible, Order No.6 - O(N!), get a task done with
N!
steps, it's terrible, Order No.7
- O(1), means your computer gets a task done with
Suppose you get notation O(N^2)
...
So by looking at Big O notation we categorize good and bad zones for algori
