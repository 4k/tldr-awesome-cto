...
inventory_code
field which is the concatenation of these three fields.
...
inventory
table in the existing schema
- Write a data migration script to split data from the existing
inventory_code
column updating the location_code
,
batch_number
and serial_number
columns.
...
Here is a change adding min_insurance_value
and
max_insurance_value
to the equipment_type
table, with some
default values.
...
This change adds some standing data to the location
and
equipment_type
tables.
...
0007_add_insurance_value_to_equipment_type.sql
and
...
flyway.table
in Flyway is used to change the name of the table
...
build.properties
file and makes changes, such as setting
db.username
to Jen
and so forth for the rest of the
settings. Once these settings are done she can just run ant
create_schema
and get a schema of her own on the team development database
...
would create a script that renames the table customer
to
client
and also creates a view
named
customer
that existing applications can use. This Parallel Change supports new and old access. It does add complexity, so
...
with the sequence number 8
, we'll see another migration with that number appear
...
migration to 9
so it will apply on top of the new migration on mainline. Once
...
migrations, including the new 8
and our renumbered 9
to a
blank database copy.
